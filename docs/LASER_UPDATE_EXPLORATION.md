...
注意本实验是**无 consolidation 的最坏情形**:tombstone 只做结果过滤,10 轮后路由图
里一半节点是墓碑 ballast;衰减同时来自边质量侵蚀与墓碑稀释。
...
4. **持续 churn 是真正的边界**:无 consolidation 时 recall 每轮缓降(见 E3 表),
   与 DiskANN 侧 fig13 的经验一致——流式更新的长期质量必须靠周期性整形
   (splice/garden/合并重建)兜底,这与"能否支持更新"是两个问题:更新原语本身已经闭合,
   整形是在这些原语之上的调度策略(full_reverse_recompute 即整行整形原语)。
