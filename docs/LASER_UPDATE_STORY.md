...
| F3 | 定宽行无 append 余量,加边⇒全行重剪枝 | 定宽行字节先付 + 纯 id 算术寻址 | 空槽 = **免费更新余量**(零增量行字节;欠满建图不损静态质量,流式插入点逼近同参数静态上界 ≤0.35pp——"反超"版已被 §11.1 对照否决,禁用);行满时行内自带码 = **免费距离估计器**(evict 补丁 ≈ 1/9 成本逼平 full RobustPrune);追加即插入,免映射表 |
...
| G3 | 行满即全行重剪枝 / 加间接层扩容 | 重剪枝 = G2 的 9×;间接层毁掉纯 id 算术寻址(加映射表、断页对齐) | 定宽行没有 append 语义,通用格式思维只有"重写"或"分裂" | E1 成本表;E1b | F3:evict 用行内码免费估距选最差边替换;headroom 欠满建图预付余量,插入点 0.9618 距同参数静态上界(R56 建,0.9644)仅 0.26pp,且远超无余量流式(evict 0.9506);"反超 R64 静态"表述已被 §11.1 否决 |
...
| A6 | "headroom 反超是不是 R56 vs R64 的参数巧合" | **已跑,攻击成立**(§11.1):R56 静态 new 段全 ef 高于 hr 插入点 0.08-0.35pp,反超系 R64 弱基线假象;主张已降级为"零字节成本逼近同参数上界"——先于审稿人自宰,defense 变 non-issue |
