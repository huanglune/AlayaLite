...
| C6 | **真 out-of-core 可更新**:匿名驻留与索引规模解耦(写池水位 + maintenance 趟内驱逐,峰值 anon ≈ 池cap+软超冲),页缓存流式承载其余——41GB deep10m 在 **4GiB** cgroup 完成 3×50k churn(consolidate+garden 全开),r3 ef100 差不限 RAM 对照 −0.007pp,RAM 地板 92GiB→≤4GiB(≥23×);退化平滑无悬崖(32→4GiB 端到端 2.49×,瓶颈=refault/读放大非 OOM) | §26 曲面(页缓存侧 20×)+ §28 六格矩阵(匿名侧拆墙) | EXPLORATION §26/§28 |
...
| 13 | RAM-cap / out-of-core(C6 证据线):**双侧已实证**——§26 页缓存侧 20×(92GiB cell recall −0.014pp、平滑 refault/PSI)+ §28 匿名侧拆墙(in-pass 水位驱逐,六格全 PASS,地板 ≤4GiB,recall 带宽 ±0.04pp);残留=refault/读放大成本优化(低水位策略、维护访问局部性、批次调度)与"RAM×吞吐"成本曲线正式图;方法论沉淀=paired A/B 位次效应 0.02-0.1pp(±0.05pp 门必须倒序平衡/随机臂序,§28.2) | C6 | 已完成(优化项为 future work) | ✓(成本曲线图随论文图表批次) |
