...
| 12 | 高维维护质量缺口(GIST960 −2.3~−3.3pp):**前沿已扫**(§20:ins400 +0.72pp/−35% 吞吐、garden 高维零效、ins600 探顶仍差 −1.38pp=剂量枯竭)⇒ **侦查收束(§22-25):侵蚀非复利**(fresh≈静态零衰减),五嫌全排除(估距误选/splice/驱逐量/garden[反而+0.75pp 保护]/补丁完整性),反修复定理(turnover 靶向 −5.4pp:hub 的机会主义积累边是承重结构,重写必须 hub 规避)⇒ 定性=**有界内在维护成本**;论文口径=前沿曲线+年龄谱法证+有界衰减+hub-aware 红线(未竟探针:incoming-route diversity,已记档) | A16 | 中(诊断 ~1h;修复未知) | **P0(高维是主场;论文口径=显式前沿曲线+归因,非单点配方)** |
