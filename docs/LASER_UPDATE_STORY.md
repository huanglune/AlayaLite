...
激进版"量化不冻结图/比 raw 更适合更新"**候选升格中**:漂移 v0 已正向(§11.2,
冻结 PQ 特异超额 −4.2pp vs LASER −1.1pp)——论文级证据(DiskANN 静态参照腿、
L 扫描、多 seed、重训 oracle)补齐后可启用激进档;措辞仍不得暗示固定 PQ 每次插入
须重训(差异在漂移下的代表性,不在能否编码)。
...
| G4 | 全局码本系的更新观(PQ/IVF:更新⇒码本漂移⇒重训练/重建) | 若 LASER 用全局码本,同样死;但 LASER 没有码本 | 逐边相对码 + 数据无关 FHT,量化没有任何全局训练产物 | **漂移实验 v0 已证**(§11.2):冻结 PQ 特异超额 −4.2pp vs LASER −1.1pp,剂量响应单调 | F1;边界:稳态 churn 下 PQ 无罪(1.0008×),优势在漂移工作负载;LASER 残余 = 拓扑冷启动(格式无关成本) |
...
| # | 攻击 | 防御 / 需补 |
|---|---|---|
| A1 | "免码本优势没有实验:稳态 SIFT churn 下 PQ 也不漂移(你们自己测的 1.0008×)" | **v0 已跑,JP-1 正向**(§11.2):同机同 trace 下冻结 PQ 漂移特异超额 −4.2pp、LASER −1.1~−2.1pp(小 2–4×),剂量响应单调;残余归因拓扑冷启动(LASER 无训练产物)。论文级仍需:DiskANN 静态参照腿、L 扫描、≥3 seeds、重训 oracle 臂 |
