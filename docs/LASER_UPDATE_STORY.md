...
激进版"量化不冻结图/比 raw 更适合更新"**降级为标题候选**:锋利但越权——在分布
漂移实验落地前,"免码本"只是机制差异不是已证优势;且不得暗示固定 PQ 码本每次插入
都须重训(它能持续编码新点,真正的差异只在漂移下的代表性 + 更新闭包不同)。
...
| F3 | 定宽行无 append 余量,加边⇒全行重剪枝 | 定宽行字节先付 + 纯 id 算术寻址 | 空槽 = **免费更新余量**(headroom 建图插入点 recall 反超静态上界);行满时行内自带码 = **免费距离估计器**(evict 补丁 ≈ 1/9 成本逼平 full RobustPrune);追加即插入,免映射表 |
...
4. **假两难陈述**:今天想要 SOTA 读路径就得接受静态索引,想要更新就得退回 raw 格式。
   ...
   (headroom 反超、splice 免 I/O、无码本可漂移)。
...
| G3 | 行满即全行重剪枝 / 加间接层扩容 | 重剪枝 = G2 的 9×;间接层毁掉纯 id 算术寻址(加映射表、断页对齐) | 定宽行没有 append 语义,通用格式思维只有"重写"或"分裂" | E1 成本表;E1b | F3:evict 用行内码免费估距选最差边替换;headroom 欠满建图预付余量,插入点 0.9618 **> 静态上界 0.9565** |
| G4 | 全局码本系的更新观(PQ/IVF:更新⇒码本漂移⇒重训练/重建) | 若 LASER 用全局码本,同样死;但 LASER 没有码本 | 逐边相对码 + 数据无关 FHT,量化没有任何全局训练产物 | 结构论证 + patch==重算单测;**分布漂移实验缺位(Gap-1,主攻实验)** | F1;诚实边界:稳态 churn 下 PQ 漂移也无罪(工业线实测 1.0008×),免码本优势要在 **分布漂移** 工作负载上才落地 |
...
| C2 | **质量有界收敛**:100% 换血 recall@100 = 0.9865/0.9878,50 轮尾段 −0.005pp/轮≈平;流式 vs 静态全建差 0.1–0.5pp;headroom 插入点反超静态上界 | E1/E1b/§9.4 长时域 | §5、§9.4 |
...
| # | 攻击 | 防御 / 需补 |
|---|---|---|
| A1 | "免码本优势没有实验:稳态 SIFT churn 下 PQ 也不漂移(你们自己测的 1.0008×)" | 承认并补 **Gap-1 分布漂移实验**(见 §6);叙事上把免码本从"recall 优势"降格为"机制优势"(永无重训练调度、无全局耦合产物),recall 优势 conditioned on shift |
...
| A4 | "更新期间的查询呢?并发读写干扰曲线缺失" | Gap-4:P3 并发统一后补"更新吞吐 × 同时查询 QPS/recall"二维图;这是系统论文必问 |
| A5 | "崩溃一致性?原地覆写 = 掉电毁图" | P3 full-page redo WAL 设计 + 开销测量(工业线同款攻击,防御方案已有) |
| A6 | "headroom 反超是不是 R56 vs R64 的参数巧合" | 已有部分防御:R56 静态建本身不输 R64(0.9728≥0.9712);补 R56 建的 1M 静态上界对照(Gap-5,便宜) |
| A7 | "evict 的估计误差长期累积?" | 估计只用于**选边**,码永远精确(patch==重算逐字节);50 轮平台 = 累积无害的直接证据;可加"evict 决策 vs 精确距离决策"一致率统计(便宜) |
...
| A11 | "full 臂不公平:逐邻居随机读没享受你们的 buffer pool/批处理,9× 是实现税不是算法税"(codex) | 给 full 同等驻留池;加 **exact-evict 臂**(只精确判最远、不做全剪枝)拆开"估距误差"与"剪枝复杂度"两个变量;9× 只描述当前实现,逻辑 I/O 复杂度另报 |
| A12 | "全部实验在 128D 免 PCA 免残差的最易路径上"(codex) | 补非 2 次幂维度数据集(强制 main_dim<dim 走 PCA 残差路径)+ npp>1 跨页几何;patch==重算的 property test 扫维度/degree/slot/编译选项 |
...
| A15 | "度量口径不一:E1 用 recall@10、churn 用 recall@100,单 seed,挑 ef 点"(codex) | 统一 recall@10/100 双报 + QPS-recall Pareto + 插入点/老点/全体三组;≥3 seeds 置信区间;固定 QPS 与固定延迟双口径 |
...
| 4 | 并发读写干扰曲线(须先做 P3 读写 fd 统一) | A4 | 高(依赖 P3) | P1 |
| 5 | headroom 公平对照:升级为二维 sweep(build_R∈{40..64} × degree_bound∈{56..80},同行字节预算),不止 R56 单点 | A6 | 低-中 | P1(codex 判 P0,裁决:先 R56 静态上界单点堵住 A6,全 sweep 随多数据集批次做) |
...
| 10 | 口径统一:≥3 seeds、recall@10/@100 双报、三组分层(插入点/老点/全体)、非 2 次幂维度数据集(走 PCA 残差路径) | A12/A15 | 中(机时为主) | P1(随规模批次一起跑) |
...
- **Abstract 六句**:① 量化盘上图 = 读路径 SOTA 但全谱系静态;可更新系统全部退守
  ...
  headroom 插入点反超静态上界。⑥ 跨格式洞见:量化格式改变退化机理(插入深度
  主杠杆 vs 入度老化)。
...
- **Fig 4**:headroom 反超(插入点 recall vs 静态上界)。
...
> 空槽是预付的更新余量(欠满建图的插入点 recall 反超静态上界),行内打包码是免费的
