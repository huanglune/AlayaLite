...
//  - dim = 128, the LaserSegmentImporter v1 floor (power-of-two, >= 128; see
//    laser_segment_importer.hpp's constructor and
//    collection_target_builder.hpp's laser_target_support()). qg's fixture
//    uses dim=64.
...
// make_unit_dataset (itself from collection_qg_seal_test.cpp), dim adjusted
// to LASER's v1 floor.
...
[[nodiscard]] auto exact_oracle_ids(const Dataset &dataset, const std::vector<float> &queries)
  ...
  return find_exact_gt<float, float, uint32_t>(
      queries, dataset.vectors, kDim, static_cast<uint32_t>(kTopK), core::Metric::l2);
}
