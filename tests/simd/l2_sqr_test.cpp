...
TEST_F(L2SqrTest, AVX2DirectCorrectnessWithTail) {
  ...
  float expected = alaya::simd::l2_sqr_generic(x.data(), y.data(), kDim);
  auto result = alaya::simd::l2_sqr_avx2(x.data(), y.data(), kDim);

  EXPECT_NEAR(result, expected, 1e-4F);
}

TEST_F(L2SqrTest, AVX512DirectCorrectnessWithMaskTail) {
  const auto &features = alaya::simd::get_cpu_features();
  ...
  float expected = alaya::simd::l2_sqr_generic(x.data(), y.data(), kDim);
  auto result = alaya::simd::l2_sqr_avx512(x.data(), y.data(), kDim);
...
TEST_F(L2SqrSQ8Test, GenericCorrectness) {
  ...
  float expected = reference_l2_sqr_sq8(x.data(), y.data(), kDim, min_vals.data(), max_vals.data());
  float result =
      alaya::simd::l2_sqr_sq8_generic(x.data(), y.data(), kDim, min_vals.data(), max_vals.data());
...
TEST_F(L2SqrSQ8Test, SimdCorrectness) {
  ...
  float expected =
      alaya::simd::l2_sqr_sq8_generic(x.data(), y.data(), kDim, min_vals.data(), max_vals.data());
  auto result = alaya::simd::l2_sqr_sq8<float, float>(x.data(),
                                                      y.data(),
                                                      kDim,
                                                      min_vals.data(),
                                                      max_vals.data());
...
TEST_F(L2SqrSQ8Test, TailHandling) {
  ...
  for (size_t dim : dims) {
    ...
    float expected =
        alaya::simd::l2_sqr_sq8_generic(x.data(), y.data(), dim, min_vals.data(), max_vals.data());
    auto result = alaya::simd::l2_sqr_sq8<float, float>(x.data(),
                                                        y.data(),
                                                        dim,
                                                        min_vals.data(),
...
TEST_F(L2SqrSQ8Test, AVX2TailHandling) {
  const auto &features = alaya::simd::get_cpu_features();
  if (!features.avx2_ || !features.fma_) {
    GTEST_SKIP() << "AVX2 + FMA not available";
  }

  const size_t kDim = 25;
  std::vector<uint8_t> x(kDim);
  std::vector<uint8_t> y(kDim);
  std::vector<float> min_vals;
  std::vector<float> max_vals;

  fill_random_uint8(x, 5);
  fill_random_uint8(y, 6);
  fill_min_max(min_vals, max_vals, kDim, 7);
...
TEST_F(L2SqrSQ4Test, GenericCorrectness) {
  ...
  float expected = reference_l2_sqr_sq4(x_packed.data(),
                                        y_packed.data(),
                                        kDim,
                                        min_vals.data(),
                                        max_vals.data());
  float result = alaya::simd::l2_sqr_sq4_generic(x_packed.data(),
                                                 y_packed.data(),
                                                 kDim,
                                                 min_vals.data(),
                                                 max_vals.data());
...
TEST_F(L2SqrSQ4Test, SimdCorrectness) {
  ...
  float expected = alaya::simd::l2_sqr_sq4_generic(x_packed.data(),
                                                   y_packed.data(),
                                                   kDim,
                                                   min_vals.data(),
                                                   max_vals.data());
  auto result = alaya::simd::l2_sqr_sq4<float, float>(x_packed.data(),
                                                      y_packed.data(),
                                                      kDim,
                                                      min_vals.data(),
                                                      max_vals.data());
...
TEST_F(L2SqrSQ4Test, ZeroVector) {
  ...
  auto result = alaya::simd::l2_sqr_sq4<float, float>(x.data(),
                                                      y.data(),
                                                      kDim,
                                                      min_vals.data(),
                                                      max_vals.data());
...
TEST_F(L2SqrSQ4Test, AVX2Correctness) {
  ...
  float expected = alaya::simd::l2_sqr_sq4_generic(x_packed.data(),
                                                   ...
                                                   min_vals.data(),
                                                   max_vals.data());
  auto result = alaya::simd::l2_sqr_sq4_avx2(x_packed.data(),
                                             y_packed.data(),
                                             kDim,
                                             min_vals.data(),
                                             max_vals.data());
...
TEST_F(L2SqrSQ4Test, AVX512Correctness) {
  ...
  float expected = alaya::simd::l2_sqr_sq4_generic(x_packed.data(),
                                                   ...
                                                   min_vals.data(),
                                                   max_vals.data());
  auto result = alaya::simd::l2_sqr_sq4_avx512(x_packed.data(),
                                               y_packed.data(),
                                               kDim,
                                               min_vals.data(),
                                               max_vals.data());
...
TEST_F(L2SqrSQ4Test, AVX512OddTailHandling) {
  const auto &features = alaya::simd::get_cpu_features();
  if (!features.avx512f_) {
    GTEST_SKIP() << "AVX-512 not available";
  }

  const size_t kDim = 33;
  std::vector<uint8_t> x_vals;
  std::vector<uint8_t> y_vals;
  std::vector<uint8_t> x_packed;
  std::vector<uint8_t> y_packed;
  ...
  fill_random_sq4(x_vals, kDim, 9);
  fill_random_sq4(y_vals, kDim, 10);
  pack_sq4(x_packed, x_vals);
  pack_sq4(y_packed, y_vals);
  fill_min_max(min_vals, max_vals, kDim, 11);
