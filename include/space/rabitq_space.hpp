...
class RaBitQSpace {
 private:
  ...
  std::unique_ptr<Rotator<DataType>> rotator_;

  IDType ep_;

  void initialize_offsets() {
    size_t rvec_len = dim_ * sizeof(DataType);
    size_t nei_quant_code_len = get_padded_dim() * kDegreeBound / 8;
 ...
 public:
  ...
  constexpr static size_t kDegreeBound = 32;
  ...
  auto prefetch_by_id(IDType id) -> void {
    mem_prefetch_l1(get_data_by_id(id), quant_codes_offset_ / 64);
  }

  auto prefetch_by_address(DataType *address) -> void {
  ...
  struct QueryComputer {
   private:
    const char *storage_ptr_;
    size_t data_chunk_size_;
    size_t qc_offset_;
    size_t f_add_offset_;
    size_t f_rescale_offset_;
    size_t nei_id_offset_;
