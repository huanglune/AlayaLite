...
size_t replay_validate_row_patch_geometry(const SegmentOp &op, bool touched_pid_evidence = false) {
...
void validate_flip_transition(const QGSuperblockV2 &image,
  ...
  if (active_superblock_slot_ < 0 || static_cast<int>(target_slot) != 1 - active_superblock_slot_) {
...
void apply_prepared_replay_flip(const PreparedReplayFlip &prepared, uint64_t &committed_watermark) {
