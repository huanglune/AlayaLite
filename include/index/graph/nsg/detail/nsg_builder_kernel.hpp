...
struct NsgBuilderKernel {
  ...
  /**
   * @brief Constructor for the detail-only NSG build kernel.
   *
   * Initializes the NSG build kernel with the given distance space and optional parameters.
   *
   ...
   */
  explicit NsgBuilderKernel(std::shared_ptr<DistanceSpaceType> &space,
                            uint32_t R = 32,
                            uint32_t L = 200)
      : space_(space),
        dim_(space->get_dim()),
