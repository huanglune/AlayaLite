...
constexpr auto lowbit(size_t x) -> size_t {
  using signed_t = std::make_signed_t<size_t>;
  return x & static_cast<size_t>(-static_cast<signed_t>(x));
}
...
// NOLINTBEGIN(portability-simd-intrinsics)
  ...
  const __m512i kLoMask = _mm512_set1_epi8(0x0f);
  ...
  for (size_t i = 0; i < code_length; i += 64) {
    ...
    lo = _mm512_and_si512(c, kLoMask);
    hi = _mm512_and_si512(_mm512_srli_epi16(c, 4), kLoMask);
  ...
  const __m256i kLowMask = _mm256_set1_epi8(0xf);
  ...
  for (size_t i = 0; i < code_length; i += 64) {
    ...
    lo = _mm256_and_si256(c, kLowMask);
    hi = _mm256_and_si256(_mm256_srli_epi16(c, 4), kLowMask);
