...
struct OngoingSlot {
    ...
    uint32_t gen;      // 4B: generation tag for O(1) invalidation  // NOLINT(readability-identifier-naming)
};
...
class FixedRingBuffer {
   public:
    ...
    explicit FixedRingBuffer(size_t capacity)
...
class FixedStack {
   public:
    ...
    explicit FixedStack(size_t capacity)
        : data_(capacity), capacity_(capacity) {}
...
struct LaserSearchParams {
    ...
    size_t aio_events_per_thread = 0;               // NOLINT(readability-identifier-naming) 0 = auto (2 * beam_width)

    [[nodiscard]] auto effective_aio_events() const -> size_t {
        return aio_events_per_thread > 0 ? aio_events_per_thread : 2 * beam_width;
    }
};
