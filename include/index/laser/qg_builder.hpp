...
class QGBuilder {
   ...
   public:
    ...
    void build(const char* vamana_file, const char* filename) {
        ...
        size_t vector_tmp_page_size =
            (d * sizeof(float) + kSectorLen - 1) / kSectorLen * kSectorLen;
        ...
        for (size_t i = 0; i < qg_.num_points_; i++) {
            ...
            vector_input.read(
                reinterpret_cast<char*>(buffer.data()), d * sizeof(float)
            );
            tmp_output.write(
                buffer.data(),
                static_cast<std::streamsize>(vector_tmp_page_size)
            );
        }
        vector_input.close();
        tmp_output.close();
        ...
        ConcurrentQueue<ThreadDate> thread_data;
...
#pragma omp critical
            {
                ...
                // Build uses its own scratch — not the search buffers
                data.sector_scratch_ = reinterpret_cast<char*>(
                    memory::align_allocate<kSectorLen>(qg_.page_size_)
                );
                data.pca_query_scratch_ = reinterpret_cast<float*>(
                    memory::align_allocate<kSectorLen>(
                        qg_.degree_bound_ * vector_tmp_page_size
                    )
                );
...
#pragma omp parallel for schedule(dynamic)
        for (size_t i = 0; i < qg_.num_points_; ++i) {
            ...
            ThreadDate data = thread_data.pop();
            while (data.sector_scratch_ == nullptr) {
                thread_data.wait_for_push_notify();
                data = thread_data.pop();
            }

            // Build uses sector_scratch_ as cur_page_scratch_ and
            // pca_query_scratch_ as neighbor_vector_scratch_
            char* cur_page = data.sector_scratch_;
            char* neighbor_buf = reinterpret_cast<char*>(data.pca_query_scratch_);
            ...
            PID* neighbor_ptr = reinterpret_cast<PID*>(
                cur_page + qg_.neighbor_offset_ * 4
            );
            ...
            size_t full_page_size =
                (full_dim * sizeof(float) + kSectorLen - 1) / kSectorLen * kSectorLen;
            ...
            reqs.reserve(cur_degree + 1);

            for (size_t j = 0; j < cur_degree; ++j) {
                auto nid = new_neighbors_[i][j].id;
                reqs.emplace_back(
                    nid * full_page_size, full_page_size, nid,
                    reinterpret_cast<void*>(neighbor_buf + j * full_page_size)
                );
            }
            reqs.emplace_back(
                i * full_page_size, full_page_size, i,
                reinterpret_cast<void*>(cur_page)
            );

            vector_reader.read(reqs, data.ctx_);

            for (size_t j = 0; j < cur_degree; ++j) {
                const auto* vec = reinterpret_cast<const float*>(
                    neighbor_buf + j * full_page_size
                );
            ...
            auto* fac_ptr = reinterpret_cast<float*>(
                cur_page + 4 * qg_.factor_offset_
            );
            auto* packed_code_ptr = reinterpret_cast<uint8_t*>(
                cur_page + 4 * qg_.code_offset_
            );
            float* triple_x = fac_ptr;
            float* factor_dq = triple_x + qg_.degree_bound_;
            float* factor_vq = factor_dq + qg_.degree_bound_;
            rabitq_codes(
                x_rotated, c_rotated, packed_code_ptr,
                triple_x, factor_dq, factor_vq
            );

            // Add residual dimension norms
            for (size_t j = 0; j < cur_degree; ++j) {
                const auto* nd = reinterpret_cast<const float*>(
                    neighbor_buf + j * full_page_size
                );
...
#pragma omp critical
            {
                output.seekp(
                    static_cast<std::streamoff>(i * qg_.page_size_ + kSectorLen),
                    std::ios::beg
                );
                output.write(
                    reinterpret_cast<const char*>(cur_page),
                    static_cast<std::streamsize>(qg_.page_size_)
                );
            }
        ...
        auto cache_num = static_cast<size_t>(
            static_cast<double>(qg_.num_vertices()) * kCacheRatio
        );
        ...
        cache_ids_out.write(
            reinterpret_cast<const char*>(&cache_num), sizeof(size_t)
        );
        cache_ids_out.write(
            reinterpret_cast<const char*>(qg_.cache_ids_.data()),
        ...
        cache_nodes_out.write(
            reinterpret_cast<const char*>(&qg_.node_len_), sizeof(size_t)
        );

        size_t batch_size = 1024;
        char* cache_buffer = reinterpret_cast<char*>(
            memory::align_allocate<kSectorLen>(batch_size * qg_.page_size_)
        );
        ...
        for (size_t i = 0; i < cache_num; i += batch_size) {
            ...
            cache_reader.read(cache_reqs, cache_reader.get_ctx());
            cache_reqs.clear();
...
inline void QGBuilder::init_from_vamana(const std::string& filename) {
    ...
    std::sort(qg_.cache_ids_.begin(), qg_.cache_ids_.end(),
              [&](PID a, PID b) { return in_degrees[a] > in_degrees[b]; });
