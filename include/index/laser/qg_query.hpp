...
class QGQuery {
 ...
 public:
  explicit QGQuery(const float *q, size_t padded_dim) : query_data_(q), padded_dim_(padded_dim) {}
   ...
   */
  void query_prepare(const FHTRotator &rotator, const QGScanner &scanner, LaserSearchContext &ctx) {
    ...
    scalar::quantize(byte_query, rd_query, padded_dim_, lower_val_, width_, sumq_);
