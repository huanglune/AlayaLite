...
class LaserIndex {
 public:
  ...
  void load(const std::string &prefix,
    ...
    graph_ = std::make_unique<symqg::QuantizedGraph>(num_points, degree, main_dim, full_dim);
  ...
  void set_search_params(const symqg::LaserSearchParams &params) {
    ...
    graph_->set_params(params.ef_search, params.num_threads, static_cast<int>(params.beam_width));
  }
   ...
   */
  void batch_search(const float *queries, size_t num_queries, uint32_t k, uint32_t *results) {
    if (!loaded_) {
      throw std::runtime_error("LaserIndex: not loaded");
    }
