...
"""Goldens for status translation and true read-only handles."""
...
def _create_closed_collection(sdk, root, flat_config):
...
def test_read_only_connect_does_not_change_any_collection_bytes(sdk, flat_config, tmp_path):
    root = tmp_path / "database"
    _create_closed_collection(sdk, root, flat_config)
    before = _directory_bytes_hash(root)

    with sdk.connect(root, read_only=True) as database:
