...
"""Goldens for add/replace/upsert/delete mutation semantics."""
...
def test_three_write_verbs_share_one_keyword_only_signature(sdk):
    expected = (
        "self",
        "ids",
        "vectors",
    ...
    for name in ("add", "replace", "upsert"):
        signature = inspect.signature(getattr(sdk.Collection, name))
